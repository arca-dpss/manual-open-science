...
> *“But it works on my computer!?!”* -- Anonymous Programmer
...
Once the container is initialized, a new interactive session will start in our terminal. Now, we are inside our container. Note the `#` prompt symbol, instead of `$`, indicating that we are root users (see Section \@ref(files-permissions)). We can check whether our project files were copied in the `home/` directory and end the session by running `exit`. 
